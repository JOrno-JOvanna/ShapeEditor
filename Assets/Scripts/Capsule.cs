...
public class Capsule : ShapeClass
{
    ...
    private Mesh CreateSemiSphere(float radius)
    {
        ...
        if (radius > 0)
        {
            ...
            for (int i = 0; i <= Sides; i++)
            {
                float longtitude = i * Mathf.PI / Sides;
                float sinLongtitude = Mathf.Sin(longtitude);
                float cosLongtitude = Mathf.Cos(longtitude);

                for (int j = 0; j <= Sides; j++)
                {
                    float latitude = j * 2 * Mathf.PI / Sides;
                    float sinLatitude = Mathf.Sin(latitude);
                    float cosLatitude = Mathf.Cos(latitude);

                    Vector3 vertex = new Vector3(radius * cosLongtitude * cosLatitude, rad * sinLongtitude, radius * cosLongtitude * sinLatitude);
                    vertices.Add(vertex);
                }
            }
        }
